 ...
 */
export async function getETHBalance(addressOrEns: string, network = 'ethereum', blockTag?: 'latest' | bigint) {
  const address = await resolveAddress(addressOrEns, network);
  const wei = await getPublicClient(network).getBalance({ address, blockNumber: typeof blockTag === 'bigint' ? blockTag : undefined });
